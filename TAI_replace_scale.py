            ...
            try:
                ...
                if max_val < 0.8:
                    with open('val_lower_than_0point8.txt', mode='a') as F:
                        F.write(os.path.basename(filename) + ' Max_val = ' + str(max_val) + '\n')
                ...
                with Image.open(filename) as base_image, Image.open('./scale/dpi300_alpha.tif') as scale_original_4c, Image.open('./scale/dpi600.tif') as watermark:
                    ...
                    transparent.save('./' + OutputPath + '/' + folder + '/' + os.path.basename(filename), compression='tiff_jpeg', quality=100, dpi=(300, 300))
